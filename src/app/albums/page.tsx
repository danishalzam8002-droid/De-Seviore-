...
import { Camera, Download, ExternalLink, Loader2, Lock } from "lucide-react";
...
function AlbumsPage() {
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      if (user) {
        try {
          const currentIsUnlocked = new Date() >= GRADUATION_DATE;
          setIsUnlocked(currentIsUnlocked);
          ...
          const userIsAdmin = adminData?.role === 'Admin' || adminData?.role === 'Admin Utama';
          setIsAdmin(userIsAdmin);

          if (currentIsUnlocked || userIsAdmin) {
            await fetchAlbums();
          }
        } catch (error) {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'INSERT') {
            ...
            setAlbums(prev => prev.map(album => album.id === payload.new.id ? payload.new : album));
          }
        }
      )
      .subscribe();
  ...
  }, [user, authLoading, router, isAdmin, isUnlocked]);
      ...
      <div className="container mx-auto px-6 py-20">
        ...
        {isLoading ? (
          ...
          <div className="max-w-4xl mx-auto">
            <div className="glass-card p-8 md:p-12 rounded-[2.5rem] border-white/10 text-center space-y-8 bg-background/40 backdrop-blur-3xl">
               <div className="inline-flex p-5 rounded-full bg-accent/10 text-accent border border-accent/20 mb-4">
                  <Lock size={48} className="animate-pulse" />
               </div>
               <h2 className="text-3xl md:text-5xl font-headline font-bold text-white uppercase tracking-tighter">
                  Halaman Masih Terkunci
               </h2>
               <p className="text-muted-foreground text-lg max-w-xl mx-auto italic">
                  "Sabar ya Akang & Teteh, Album Kenangan kita akan terbuka secara otomatis di waktu wisuda tiba! Persiapkan diri untuk nostalgia bersama. ✨"
               </p>
               <div className="pt-8 border-t border-white/5">
                  <AlbumCountdown targetDate={GRADUATION_DATE} onComplete={() => setIsUnlocked(true)} />
               </div>
               <Button variant="ghost" className="mt-8 text-accent hover:bg-accent/10" onClick={() => router.push("/")}>
                  KEMBALI KE BERANDA
               </Button>
            </div>
          </div>
          ...
          <>
            {isAdmin && !isUnlocked && (
              <div className="max-w-fit mx-auto mb-10 px-6 py-2 rounded-full bg-yellow-500/10 border border-yellow-500/20 text-yellow-500 text-xs font-bold uppercase tracking-widest flex items-center gap-2">
                <span className="relative flex h-2 w-2">
                  <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-yellow-400 opacity-75"></span>
                  <span className="relative inline-flex rounded-full h-2 w-2 bg-yellow-500"></span>
                </span>
                Admin Preview Mode - Belum Terbuka untuk Umum
              </div>
            )}
            <Carousel
              opts={{
